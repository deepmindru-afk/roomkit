...
audio was read to the end, so a generation cut off by a barge-in never
provides one.
"""
...
class RequestIdLedger:
    """The ``request_id`` of each fully read generation, per voice session."""

    def __init__(self, clock: Callable[[], float] = time.monotonic) -> None:
        self._clock = clock
        self._ids: dict[str, dict[str, tuple[str, float]]] = {}

    def record(self, context_id: str, turn_id: str, request_id: str) -> None:
        """Keep *request_id* as the id of the turn *turn_id* will become."""
        ids = self._ids.setdefault(context_id, {})
        ids[turn_id] = (request_id, self._clock())
        for stale in list(ids)[:-MAX_REQUEST_IDS]:
            del ids[stale]

    def forget(self, context_id: str) -> None:
        self._ids.pop(context_id, None)

    def stitching_params(self, context: TTSContext) -> dict[str, Any]:
        """The stitching arguments for the next generation of *context*.

        Continuity follows the last thing the user heard: after a cut-off
        turn nothing is sent, and the response starts afresh.
        """
        ...
        for turn in reversed(turns):
            ...
            if turn.interrupted or entry is None or now - entry[1] > REQUEST_ID_TTL_S:
                break
