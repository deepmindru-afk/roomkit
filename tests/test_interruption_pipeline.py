...
class TestSemanticBackchannelIsDiscarded:
    async def test_backchannel_during_playback_never_reaches_stt(self) -> None:
        """RFC §12.6 step 5 — a speech segment rejected as a backchannel is
        captured but discarded: the "mm-hmm" must not become a user message."""
        ...
        detector = MockBackchannelDetector(
            decisions=[BackchannelDecision(is_backchannel=True, label="acknowledgement")]
        )
        kit, channel, session = await _voice_room(
            InterruptionConfig(
        ...
        backchannels: list[object] = []

        @kit.hook(HookTrigger.ON_BACKCHANNEL, HookExecution.ASYNC)
        async def on_backchannel(event, context):
