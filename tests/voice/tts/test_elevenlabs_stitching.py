...
class TestLedger:
    def test_no_turn_sends_nothing(self) -> None:
        assert RequestIdLedger().stitching_params(_context()) == {}

    def test_heard_turns_send_their_ids_oldest_first(self) -> None:
        ledger = RequestIdLedger()
        for i in range(4):
            ledger.record("s1", f"t{i}", f"r{i}")

        params = ledger.stitching_params(_context(*(_turn(f"t{i}") for i in range(4))))
    ...
    def test_a_cut_off_last_turn_sends_nothing(self) -> None:
        ledger = RequestIdLedger()
        ledger.record("s1", "t0", "r0")

        params = ledger.stitching_params(_context(_turn("t0"), _turn("t1", interrupted=True)))
    ...
    def test_ids_stop_at_an_interrupted_turn(self) -> None:
        ...
        for turn_id in ("t0", "t1", "t2"):
            ledger.record("s1", turn_id, f"r-{turn_id}")

        params = ledger.stitching_params(
            _context(_turn("t0"), _turn("t1", interrupted=True), _turn("t2"))
        )

        assert params == {"previous_request_ids": ["r-t2"]}

    def test_a_turn_without_id_falls_back_to_its_text(self) -> None:
        params = RequestIdLedger().stitching_params(_context(_turn("t0", "Hello there.")))
    ...
    def test_an_expired_id_falls_back_to_text(self) -> None:
        ...
        params = ledger.stitching_params(_context(_turn("t0", "Old.")))
    ...
    def test_user_turns_are_never_sent(self) -> None:
        ...
        assert RequestIdLedger().stitching_params(_context(user)) == {}
    ...
    def test_forget_drops_the_session(self) -> None:
        ...
        assert ledger.stitching_params(_context(_turn("t0", "x"))) == {"previous_text": "x"}
...
def _provider(**config: Any) -> tuple[ElevenLabsTTSProvider, list[dict[str, Any]]]:
    ...
    async def raw_stream(**kwargs: Any) -> AsyncIterator[_RawResponse]:
        calls.append(kwargs)
        yield _RawResponse(f"req-{next(counter)}", [b"\x00\x01", b"\x02\x03"])
...
class TestProvider:
    ...
    async def test_a_stream_cut_off_keeps_no_id(self) -> None:
        provider, calls = _provider()

        stream = provider.synthesize_stream("One.", context=_context(next_turn_id="t0"))
        await anext(stream)
        await stream.aclose()  # barge-in: the transport stops reading
        await _drain(provider.synthesize_stream("Two.", context=_context(_turn("t0", "One."))))
